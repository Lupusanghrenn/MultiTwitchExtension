...
function init() {
	//TODO ne faire que les 100 premiers (favoris)
	...
	if (favoritesChannel!=undefined && favoritesChannel[0]!=undefined) {
		for(var i=0;i<favoritesChannel;i++){
			chaine+=favoritesChannel[i]+"&user_login=";
			length++;
		}
	}
	for (var i = 0; i < urls.length && length<100; i++) {
		if(!favoritesChannel.includes(urls[i])){
